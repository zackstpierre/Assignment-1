...
for i = 1:1000
   ...
   Vy(P_scatter_locations) = randn(sum(P_scatter_locations),1) * Vth;
   Vx(P_scatter_locations) = randn(sum(P_scatter_locations),1) * Vth;
...
V_histogram = sqrt(Vx.^2 + Vy.^2);
figure(3)
histogram(V_histogram)
title('Distribution of Velocities')
xlabel('Velocity (m/s)')
ylabel('Number of Particles at Given Velocity')