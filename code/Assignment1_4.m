...
r = 25e-9;
pos = [size_x/2-r size_y/2-r r*2 r*2];
...
P_inside_circle = (Px - size_x/2-r).^2 + (Py - size_y/2-r).^2 < r^2; 

while sum(P_inside_circle) > 0
    Px(P_inside_circle) = rand(1)*size_x;
    Py(P_inside_circle) = rand(1)*size_y;
...
for i = 1:1000
   ...
   normal_x = Px(P_inside_circle) - size_x/2;
   normal_y = Px(P_inside_circle) - size_y/2;

   phi = atan2d(normal_x.*Py(P_inside_circle) - normal_y.*Px(P_inside_circle), normal_x.*Px(P_inside_circle) + normal_y.*Py(P_inside_circle));
   
   Pxy_normal = [Px(P_inside_circle) Py(P_inside_circle)];
